...
export function UpdatePostForm({ onPostSuccess, className }: UpdatePostFormProps) {
  ...
  async function onSubmit(data: UpdateFormValues) {
    ...
    const photosDataUrls = await Promise.all(
        photos.map(photo => {
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    {photos.map((photo, index) => (
                     ...
                     <div className="flex items-center justify-center w-full">
                        <label htmlFor="dropzone-file-updates" className={`flex flex-col items-center justify-center w-full h-full aspect-square border-2 border-dashed rounded-lg ${isSubmitting ? 'cursor-not-allowed bg-muted/50' : 'cursor-pointer bg-secondary hover:bg-muted'}`}>
                            <div className="flex flex-col items-center justify-center text-center p-2">
                                <Upload className="w-8 h-8 mb-2 text-muted-foreground" />
                                <p className="text-xs text-muted-foreground"><span className="font-semibold">Add photos</span></p>
                            </div>
                            <Input id="dropzone-file-updates" type="file" className="hidden" onChange={handlePhotoUpload} accept="image/png, image/jpeg, image/webp" multiple disabled={isSubmitting} />
                        </label>
                    </div> 
                </div>
            </div>
